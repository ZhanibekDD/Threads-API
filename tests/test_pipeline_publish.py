...
def _insert_post_and_reply(conn, approved=0):
    conn.execute(
        ...
        "found_at, status) VALUES "
        "('p1', 'author1', 'user1', 'text', 'link', ?, 'queued_for_review')",
        (now_iso(),),
    )
    post_id = conn.execute(
        "SELECT id FROM threads_posts WHERE threads_post_id = 'p1'"
    ).fetchone()["id"]
    conn.execute(
        "INSERT INTO generated_replies "
        "(post_id, generated_text, model, prompt_version, approved, "
        "published, created_at) VALUES (?, 'draft comment', 'model', "
        "'v1', ?, 0, ?)",
        (post_id, approved, now_iso()),
    )
    conn.commit()
    return conn.execute(
        "SELECT id FROM generated_replies WHERE post_id = ?",
        (post_id,),
    ).fetchone()["id"]
...
@pytest.mark.asyncio
async def test_dry_run_never_calls_api_or_marks_published(conn):
    reply_id = _insert_post_and_reply(conn, approved=1)
    object.__setattr__(config, "dry_run", True)
    threads = AsyncMock()

    result = await publish_approved_reply(
        threads=threads,
        conn=conn,
        reply_id=reply_id,
    )
...
async def test_successful_publish_updates_author_and_reply(conn):
    ...
    conn.execute(
        "INSERT INTO authors "
        "(author_id, username, first_seen, replies_count) "
        "VALUES ('author1', 'user1', ?, 0)",
        (now_iso(),),
    )
    conn.commit()
    ...
    author = conn.execute(
        "SELECT * FROM authors WHERE author_id = 'author1'"
    ).fetchone()
...
async def test_already_published_is_idempotent(conn):
    reply_id = _insert_post_and_reply(conn, approved=1)
    conn.execute(
        "UPDATE generated_replies SET published = 1, "
        "threads_reply_id = 'old_id' WHERE id = ?",
        (reply_id,),
    )
    conn.commit()
    threads = AsyncMock()
