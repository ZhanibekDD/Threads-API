...
def test_daily_briefs_create_two_threads_posts_and_repurpose_topics():
...
def test_odd_day_uses_russian_second_post_and_no_carousel():
...
async def test_generation_saves_funnel_metadata(conn):
    texts = [
        "Счёт заблокирован? Сначала выясните основание. Код АРЕСТ.",
        "Запрет на выезд проверяют заранее. Код ВЫЕЗД.",
        "Видео о первом шаге после ареста счёта.",
        "Сторис: вопрос, ответ и код АРЕСТ.",
        "Сторис о запрете на выезд и коде ВЫЕЗД.",
        "Карусель о проверке запрета на выезд.",
    ]
    plan = await generate_daily_content_plan(
        _deepseek_with_responses(*texts),
        conn,
        day_of_year=200,
    )
...
async def test_duplicate_generation_is_rewritten_once(conn):
    ...
    rewritten = (
        "Не знаете, кто заблокировал счёт? Получите постановление и "
        "сверьте взыскателя. Код АРЕСТ."
    )
    deepseek = _deepseek_with_responses(duplicate, rewritten)

    with patch(
        "app.content_generator.build_daily_briefs",
        return_value=[brief],
    ):
        plan = await generate_daily_content_plan(
            deepseek,
...
@pytest.mark.asyncio
async def test_publish_content_by_id_marks_real_post_published(conn):
    conn.execute(
...
@pytest.mark.asyncio
async def test_rejected_content_is_never_published(conn):
    conn.execute(
        "INSERT INTO own_content "
        "(type, language, content, created_at, rejected) "
        "VALUES ('post', 'ru', 'Отклонённый пост', ?, 1)",
        (now_iso(),),
    )
    conn.commit()
    content_id = conn.execute("SELECT id FROM own_content").fetchone()["id"]
