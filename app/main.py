...
def _threads_client(conn) -> ThreadsClient:
    """Use the DB-stored OAuth token before the environment fallback."""
...
async def cmd_generate_content() -> None:
    ...
    print(f"Сгенерировано {len(plan)} материалов, из них постов Threads: {posts}.")
    ...
    with session() as conn:
        ...
        for row in rows:
            if row["rejected"]:
                ...
                flag = "needs approval"
            print(
                f"\n[{row['id']}] {row['type']} ({row['language']}) — {flag} "
                f"— {len(row['content'])} символов"
            )
            print(
...
async def cmd_run_content_scheduler() -> None:
    ...
    while True:
        ...
        if is_publish_time_now(times, now, config.own_content_timezone):
            ...
            slot_key = (
                f"{local_now.date().isoformat()}_{local_now.strftime('%H:%M')}"
            )
