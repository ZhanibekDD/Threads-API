...
def build_daily_briefs(day_of_year: int | None = None) -> list[ContentBrief]:
    ...
    if day_of_year is None:
        day_of_year = datetime.now(
            tz=ZoneInfo(config.own_content_timezone)
        ).timetuple().tm_yday
...
def _recent_content(conn: sqlite3.Connection, limit: int = 20) -> list[str]:
    rows = conn.execute(
        "SELECT content FROM own_content WHERE content <> '' "
...
async def generate_daily_content_plan(
    ...
    for brief in build_daily_briefs(day_of_year):
        ...
        conn.execute(
            ...
            "(type, language, content, created_at, pillar, funnel_stage, "
            "topic, cta_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                brief.content_type,
                brief.language,
                text,
...
async def _generate_one(
    ...
    if avoid_texts:
        examples = "\n---\n".join(avoid_texts[:5])
        prompt += (
            "\n\nНе повторяй структуру, крючки и формулировки этих "
            "недавних материалов:\n" + examples
        )
    if rewrite:
        prompt += (
            "\n\nПредыдущий вариант оказался слишком похожим. "
            "Полностью смени крючок и структуру."
        )
    ...
    try:
        ...
        logger.exception(
            "own_content_generation_failed type=%s",
            brief.content_type,
        )
        return ""
...
def export_content(conn: sqlite3.Connection, out_dir: str = "./exports") -> str:
    ...
    rows = conn.execute(
        "SELECT * FROM own_content WHERE exported = 0 ORDER BY id"
    ).fetchall()
    out_path = Path(out_dir) / f"content_{now_iso().replace(':', '-')}.csv"
    with open(out_path, "w", newline="", encoding="utf-8-sig") as file_handle:
    ...
    if ids:
        conn.executemany(
            "UPDATE own_content SET exported = 1 WHERE id = ?",
...
def parse_publish_times(raw: str) -> list[str]:
    """'10:00, 18:00' -> ['10:00', '18:00']. Ignores blanks."""
...
async def publish_content_by_id(
    ...
    if length > THREADS_POST_MAX_CHARS:
        ...
        conn.commit()
        logger.warning(
            "own_content_too_long id=%s length=%s",
            row["id"],
            length,
        )
        return {
    ...
    conn.commit()
    logger.info(
        "own_content_published id=%s threads_post_id=%s",
        row["id"],
        post_id,
    )
    return {
...
async def publish_next_approved(
    ...
    if not config.auto_publish_own_content:
        logger.info(
            "auto_publish_own_content_disabled_would_publish id=%s",
            row["id"],
        )
