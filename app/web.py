...
from app.content_generator import generate_daily_content_plan, publish_content_by_id
...
if not config.review_panel_secret_key:
    logger.warning(
        "REVIEW_PANEL_SECRET_KEY not set — using a random key that changes every restart."
    )
if not config.review_panel_password:
    logger.warning(
        "REVIEW_PANEL_PASSWORD not set — the review panel has NO login. "
        "Set a password before exposing it beyond localhost."
    )
...
def threads_callback():
    ...
    with db_session() as conn:
        if not oauth.consume_state(conn, state):
            ...
            abort(
                400,
                description="Недействительный или истёкший state — подключитесь заново.",
            )
        if not code:
        ...
        try:
            ...
            flash("Не удалось получить токен — проверьте логи сервера.", "error")
            return redirect(url_for("dashboard"))
...
def threads_deauthorize():
    ...
    with db_session() as conn:
        ...
        conn.commit()
    logger.info(
        "threads_deauthorize_processed user_id=%s",
...
def threads_data_deletion():
    ...
    with db_session() as conn:
        ...
        conn.commit()
    logger.info(
        "threads_data_deletion_requested user_id=%s code=%s",
        data.get("user_id", "unknown"),
        confirmation_code,
    )
...
def generate_content():
    ...
    if plan:
        ...
        flash(
            ...
            "Проверьте и одобрите их ниже.",
            "success",
        )
    else:
        flash(
            "Новые материалы не созданы. Проверьте DEEPSEEK_API_KEY и логи.",
            "error",
        )
    return redirect(url_for("dashboard"))
...
def approve_content(content_id: int):
    with db_session() as conn:
        ...
        if row is None:
            ...
            conn.execute(
                "UPDATE own_content SET approved = 1, rejected = 0 WHERE id = ?",
                (content_id,),
            )
            flash(
                f"Пост #{content_id} одобрен для ближайшего слота.",
                "success",
            )
    return redirect(url_for("dashboard"))
...
def publish_content(content_id: int):
    ...
    with db_session() as conn:
        try:
            ...
            flash(
                f"Threads API не опубликовал пост: {exc.status_code} — {exc.payload}",
                "error",
            )
            return redirect(url_for("dashboard"))

    if result["status"] == "published":
        flash(f"Пост #{content_id} опубликован в Threads.", "success")
    else:
...
def approve_reply(reply_id: int):
    with db_session() as conn:
        ...
        if result.get("status") == "published":
            ...
            flash(
                "DRY_RUN включён — ответ одобрен, но не отправлен.",
                "info",
            )
        else:
...
def reject_reply(reply_id: int):
    with db_session() as conn:
        ...
        if row is not None:
            conn.execute(
                "UPDATE threads_posts SET status = 'skipped_by_human' WHERE id = ?",
                (row["post_id"],),
            )
...
def await_(coro):
    """Run one async operation inside the small synchronous Flask panel."""
